  ...
  <UCard class="shadow-lg p-2 sm:p-4">
    <template #header>
      <div class="flex items-center justify-center gap-2 text-center w-full">
        ...
        <span v-if="isMobile" class="text-lg sm:text-3xl font-semibold">Arrivée prévue le 22 janvier</span>
        <span v-else class="text-lg sm:text-3xl font-semibold">Livraison du colis prévue le 22 janvier 2026</span>
      </div>
    </template>
      ...
      <template v-else>
        <UCard variant="subtle">
          <div class="flex flex-col items-center">
            <span class="text-4xl sm:text-5xl font-bold mb-2">{{ countdown.days.toString().padStart(2, '0') }}</span>
            <span class="text-sm uppercase tracking-wider">JOURS</span>
          </div>
        </UCard>
        <UCard variant="subtle">
          <div class="flex flex-col items-center">
            <span class="text-4xl sm:text-5xl font-bold mb-2">{{ countdown.hours.toString().padStart(2, '0') }}</span>
            <span class="text-sm uppercase tracking-wider">HEURES</span>
          </div>
        </UCard>
        <UCard variant="subtle">
          <div class="flex flex-col items-center">
            <span class="text-4xl sm:text-5xl font-bold mb-2">{{ countdown.minutes.toString().padStart(2, '0') }}</span>
